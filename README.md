...
1. **รับคำสั่ง**: เมื่อได้รับคำสั่งซื้อขายผ่าน Endpoint `/execute` ระบบจะตรวจสอบ `Idempotency-Key` (หรือ `client_order_id`) เพื่อป้องกันคำสั่งซ้ำ
...
| :--- | :--- | :--- |
| `client_order_id` | `str` | รหัสคำสั่งซื้อขายจากฝั่ง Client (ต้องไม่ซ้ำกัน) |
| `account_id` | `int` | รหัสบัญชีผู้ใช้งาน |
...
| `client_order_id` | `str` | รหัสคำสั่งซื้อขายจากฝั่ง Client |
