...
from app.services.broker_cleanup import (
...
def test_is_stale_order_detects_old_submitted_entry_order_but_not_protective_stop():
    ...
    assert is_stale_order(

        stale_entry_order(minutes=500),
    ...
    assert is_stale_order(
        ...
        protective_stop_order(minutes=500),

        max_age_minutes=390,

        position_symbols={"ACGL"},
...
async def test_cancel_stale_orders_dry_run_does_not_call_broker_cancel_or_cancel_protective_stops():
    ...
    result = await service.cancel_stale_open_orders(
