...
class BrokerStateReconciliationService:
    ...
    async def collect_broker_state(

        self,

        account_id: int | str = 1,
        ...
        order_summary = classify_open_orders(
        ...
        return {
            ...
            "summary": {
                ...
                "unknown_age_order_count": order_summary.get(

                    "unknown_age_order_count",

                    0,

                ),

                "protective_order_count": order_summary.get(
                ...
                "stale_entry_order_count": order_summary.get(

                    "stale_entry_order_count",

                    0,

                ),

                "stale_non_entry_order_count": order_summary.get(

                    "stale_non_entry_order_count",
    ...
    async def push_broker_state_to_database(
        ...
        endpoint = (
            ...
            settings.BROKER_SYNC_ENDPOINT

            if settings.BROKER_SYNC_ENDPOINT.startswith("/")

            else f"/{settings.BROKER_SYNC_ENDPOINT}"
        ...
        headers = {
            ...
            "X-API-KEY": settings.DATABASE_AGENT_API_KEY or settings.API_KEY

        }

        async with httpx.AsyncClient(

            timeout=float(settings.BROKER_SYNC_TIMEOUT_SECONDS)
            ...
            try:
                ...
                response = await client.post(
                    ...
                    json=jsonable_encoder(broker_state),
    ...
    async def reconcile(
        ...
        *,
        ...
        database_sync = (
            ...
            await self.push_broker_state_to_database(broker_state)
        ...
        if (
            ...
            settings.FAIL_RECONCILE_WHEN_DB_SYNC_FAILS

            and database_sync.get("status") == "failed"
